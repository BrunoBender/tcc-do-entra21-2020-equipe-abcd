...
public class DTOMethodProcessor extends RequestResponseBodyMethodProcessor {
	private static final DozerBeanMapper MDOEL_MAPPER = new DozerBeanMapper();
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer modelAndViewContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
		Object dto = super.resolveArgument(parameter, modelAndViewContainer, webRequest, binderFactory);
		return MDOEL_MAPPER.map(dto, parameter.getParameterType());
	}
	...
	protected Object readWithMessageConverters(HttpInputMessage inputMessage, MethodParameter parameter, Type targetType) throws IOException, HttpMediaTypeNotSupportedException, HttpMessageNotReadableException {
		for (Annotation annotation : parameter.getParameterAnnotations()) {
			RequestDTO dtoType = AnnotationUtils.getAnnotation(annotation, RequestDTO.class);
			if (dtoType != null) {
